"""Model registry — loads configuration from database, manages provider instances.

Responsibilities:
  - Load model configs from database (model_configs table)
...
class ModelRegistry:
    ...
    Singleton that reads the model_configs database table, parses all routing
    fields, exposes configs to the router, and lazily instantiates providers.
